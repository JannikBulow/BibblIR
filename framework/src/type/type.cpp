...
namespace bibblir {
    ...
    Type* Type::GetArrayType(Type* elementType) {
        return GetType<
            ArrayType,
            [](ArrayType* value, Type* elementType) { return value->getElementType() == elementType; },
            [](Type* elementType) { return std::make_unique<ArrayType>(elementType); }
        >(elementType);
    }
